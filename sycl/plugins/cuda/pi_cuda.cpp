...
pi_result cuda_piextProgramSetSpecializationConstant(pi_program Prog,
                                                     pi_uint32 SpecID,
                                                     size_t SpecSize,
  ...
  assert(DPtr && Bytes && "Invalid symbol.");

  const auto ResMemcpy = cuMemcpyHtoD(DPtr, SpecValue, SpecSize);
  return PI_CHECK_ERROR(ResMemcpy);
}
